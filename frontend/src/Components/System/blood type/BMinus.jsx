...
function BMinus() {
    ...
    const handleSendEmail = (e) => {
        e.preventDefault()
        axios.post("http://localhost:7000/bminus/emails", {
            "subject": subject,
            "text": text,
            "bloodType": bloodType,
            ...
            Swal.fire({
                ...
                text: "Email Sends all Blood type B-",
                icon: "success"
            })
            setSubject("")
            setText("")
    ...
    const handleReadBMinus = () => {
        axios.get("http://localhost:7000/bminus/donors").then((res) => {
            setBMinus(res.data)
        }).catch((error) => console.log(error))
    }
