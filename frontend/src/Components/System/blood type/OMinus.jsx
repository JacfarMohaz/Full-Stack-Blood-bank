...
function OMinus() {
    ...
    const handleSendEmail = (e) => {
        e.preventDefault()
        axios.post("http://localhost:7000/ominus/emails", {
            "subject": subject,
            "text": text,
            "bloodType": bloodType,
            ...
            Swal.fire({
                ...
                text: "Email Sends all Blood type O-",
                icon: "success"
            })
            setSubject("")
            setText("")
    ...
    const handleReadOMinus = () => {
        axios.get("http://localhost:7000/ominus/donors").then((res) => {
            setOMinus(res.data)
        }).catch((error) => console.log(error))
    }
    ...
    return <div>
        ...
        <div className="bg-fourthColor pb-4 mb-3 w-[75%] h-[70%] ml-[22%] mt-10 rounded-2xl shadow-2xl">
            <div className="flex justify-between px-2 pb-3">
                <h1 className="text-3xl text-textColor font-semibold pt-4">O- BLood</h1>
